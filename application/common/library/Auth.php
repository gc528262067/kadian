...
class Auth
{
    ...
    protected $_token = '';
    //Token默认有效时长
    protected $keeptime = 8640000;
    protected $requestUri = '';
    protected $rules = [];
    //默认配置
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'realname', 'status', 'mobile', 'avatar', 'score', 'openid'];
    ...
    {
        $field = Validate::is($account, 'email') ? 'email' : (Validate::regex($account, '/^1\d{10}$/') ? '' : 'username');
        $user = User::get(['mobile' => $account]);
        if (!$user) {
//            $this->setError('Account is incorrect');
            $this->setError('找不到该用户');
            return false;
        }

        if ($user->status != 'normal') {
            $this->setError('账户已被封禁');
            return false;
        }
    ...
    public function getUserinfo()
    {
        ...
        $userinfo['token'] = Token::get($this->_token)['token'];
        $userinfo['expires_in'] = Token::get($this->_token)['expires_in'];
        return $userinfo;
    }
